...
\node [rblock]  (radloop) {Boiler, HP \& TES\\ loop control};
...
\node [decision, below of=less18, node distance=3 cm] (lesssuptemp) {$<$Sup temp};
...
\node [block, right of=less2, node distance=5 cm] (hpon) {Turn HP on};

\node [block, below of=outsidetemp, node distance=2 cm] (hpboioff) {Turn off HP\\ \& Boiler};

\node [decision, left of=boion, node distance=5 cm] (greatsup) {$>$Sup temp};
